...
final class ThemeServiceProvider extends ServiceProvider
{
    ...
    public function enqueueAssets(): void
    {
        ...
        if (is_readable($distManifest)) {
            wp_enqueue_style('dnorte-theme-app', DNORTE_THEME_URI . '/dist/app.css', [], DNORTE_THEME_VERSION);
        }

        $distScript = DNORTE_THEME_DIR . '/dist/app.js';

        if (is_readable($distScript)) {
            wp_enqueue_script('dnorte-theme-app', DNORTE_THEME_URI . '/dist/app.js', [], DNORTE_THEME_VERSION, true);
        }
    }
}
