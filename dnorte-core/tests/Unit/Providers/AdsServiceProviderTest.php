<?php
/**
 * renderCabecera()/renderInicio()/enqueueAdSenseLoader()/injectArticleAds()/
 * renderTrackingScript() en sí (la resolución de CampaignRepository vía el
 * contenedor y su comportamiento real) no se cubren aquí a propósito — ver el
 * docblock de AdsServiceProvider: dependen en cadena de wpdb, inexistente en este
 * proceso de pruebas. Cubierto de punta a punta por `tests/Integration/Ads/*`.
 *
...
final class AdsServiceProviderTest extends TestCase {
	...
	public function test_boot_wires_the_sitewide_article_and_tracking_hooks(): void {
	...
	public function test_add_rest_controllers_appends_the_campaign_event_controller(): void {
		$container = new Container();

		$result = ( new AdsServiceProvider( $container ) )->addRestControllers( array() );

		self::assertSame( array( CampaignEventController::class ), $result );
	}
}
